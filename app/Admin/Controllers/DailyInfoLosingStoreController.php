...
class DailyInfoLosingStoreController extends AdminController
{
    /**
    ...
    protected function grid()
    {
        ...
        if (Admin::user()->isRole('administrator') || Admin::user()->isRole('master')) {
            ...
            if ($admin_role_slug === 'store') {
                $grid->model()->where('recommends.step4_id', Admin::user()->id);
                $grid->model()->whereNull('recommends.step5_id');
            } else {
                //상위 계정이라면
        ...
        $grid->column('하부갯수')->display(function (){
            $cnt = Recommend::where('step4_id', $this->id)->count();
            return $cnt;
        });
    ...
    protected function detail($id)
    {
        ...
        $show->losingTotalCompany('루징내역', function ($losingTotalCompany){
            ...
            $losingTotalCompany->total_deposit('충전된 유저 포인트')->display(function ($total_deposit){
            ...
            $losingTotalCompany->footer(function ($query){
                $data = $query->max('user_id');
                if ((int)$data === (int)Admin::user()->id) {
